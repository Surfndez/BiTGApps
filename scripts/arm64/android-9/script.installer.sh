...
#
# Updated on      : Saturday November 30 11:23:12 IST 2019
#
...
ZIP="
  ...
  zip/core/priv_app_PrebuiltGmsCorePi.tar.gz
  zip/sys/sys_app_FaceLock.tar.gz
  ...
  zip/sys/sys_app_MarkupGoogle.tar.gz
...
extract_app() {
  ...
  tar tvf $ZIP_FILE/sys/sys_app_FaceLock.tar.gz >> $LOG
  ...
  tar tvf $ZIP_FILE/sys/sys_app_MarkupGoogle.tar.gz >> $LOG
  tar tvf $ZIP_FILE/sys/sys_app_SoundPickerPrebuilt.tar.gz >> $LOG
  tar -xz -f $ZIP_FILE/sys/sys_app_FaceLock.tar.gz -C $TMP_SYS
  ...
  tar -xz -f $ZIP_FILE/sys/sys_app_MarkupGoogle.tar.gz -C $TMP_SYS
  ...
  tar tvf $ZIP_FILE/core/priv_app_PrebuiltGmsCorePi.tar.gz >> $LOG
  ...
  tar -xz -f $ZIP_FILE/core/priv_app_PrebuiltGmsCorePi.tar.gz -C $TMP_PRIV
  echo "- Done" >> $LOG
  echo "-----------------------------------" >> $LOG
